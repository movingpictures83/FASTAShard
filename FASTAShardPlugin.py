...
class FASTAShardPlugin:
  def input(self, inputfile):
    self.parameters = PyIO.readParameters(inputfile)

    self.fastaFileToSplit = PyPluMA.prefix()+"/"+self.parameters["fastafile"]
    self.numberOfPartitions = int(self.parameters["partitions"])
    self.affixForOutputFile = self.parameters["affix"]
    self.versionForOutputFile = self.parameters["version"]
  ...
  def output(self, output_directory):
    ...
    #self.fastaFileToSplit     = args.file
    #self.numberOfPartitions    = args.partitions
    #self.affixForOutputFile   = args.affix if args.affix is not None else "partition"
    #self.versionForOutputFile = args.version if args.affix is not None else "007"
    ...
    if not os.path.exists( output_directory ):
        ...
        output_directory = output_directory + "_" + str(self.numberOfPartitions)
    ...
    print("[ " + time.strftime('%d-%b-%Y %H:%M:%S', time.localtime()) + " ]")
    print("[ " + time.strftime('%d-%b-%Y %H:%M:%S', time.localtime()) + " ] File to Split: " + self.fastaFileToSplit )
    print("[ " + time.strftime('%d-%b-%Y %H:%M:%S', time.localtime()) + " ] No. of Partitions: " + str(self.numberOfPartitions) + "")
    print("[ " + time.strftime('%d-%b-%Y %H:%M:%S', time.localtime()) + " ] Affix: " + self.affixForOutputFile)
    print( "[ " + time.strftime( '%d-%b-%Y %H:%M:%S', time.localtime() ) + " ] Version: " + self.versionForOutputFile )
    ...
    listOfRecordsInFastaFile = list(SeqIO.parse(self.fastaFileToSplit, "fasta"))
    ...
    batchSizeForPartitions = math.ceil(totalNumberOfFASTARecords / self.numberOfPartitions)
    ...
    sizeOfFastaFile = os.path.getsize( self.fastaFileToSplit )
    ...
    #
    record_iter = SeqIO.parse( open( self.fastaFileToSplit ), "fasta" )
    ...
    for i, batch in enumerate(batch_iterator(record_iter, batchSizeForPartitions)):
        ...
        outputFilename = output_directory_for_partition + "/" + self.affixForOutputFile + "_v" + self.versionForOutputFile + ".fasta"
